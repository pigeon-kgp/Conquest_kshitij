...
def draw():
    glabal riverList, area
    s=int(sqrt(area)/2)
    for i in riverList:
        x=int(i[0])
        y=int(i[1])
        try: img[y+(s+10),x+(s+10)]=[0,0,255]
        except: pass
        try: img[y+(s+10),x-(s+10)]=[0,0,255]
        except: pass
        try: img[y-(s+10),x+(s+10)]=[0,0,255]
...
def plot():
    global woodList, foodList
    for i in woodList:
        img[int(i[1]),int(i[0])]=[0,255,0]
        img[int(i[1])+1,int(i[0])+1]=[0,255,0]
        img[int(i[1])+1,int(i[0])]=[0,255,0]
        img[int(i[1]),int(i[0]+1)]=[0,255,0]
    for i in foodList:
        img[int(i[1]),int(i[0])]=[0,255,0]
        img[int(i[1])+1,int(i[0])+1]=[0,255,0]
        img[int(i[1])+1,int(i[0])]=[0,255,0]
...
myfile=file.open("river.txt","r")
while(1):
    line=file.readline()
    if not line: break
    riverList.append(line.split(',')[0],line.split(',')[1])
myfile.close()

myfile=file.open("wood.txt","r")
while(1):
    line=file.readline()
    if not line: break
